  ...
  return <>
    ...
    <main>
      ...
      <section className="explorer" aria-label="Explore spouse occupations">
        ...
        <div className="results-heading"><div><div className="eyebrow">FOLLOW THE CONNECTIONS</div><h2>{gender === 'Woman' ? 'Women' : gender === 'Man' ? 'Men' : 'Non-binary people'} in <span>{occupation.toLowerCase() === 'teacher' ? 'teaching' : occupation.toLowerCase()}</span></h2><p>Spouses grouped by occupation and gender.</p></div><div className="view-switch" role="group" aria-label="Results view"><button aria-pressed={view === 'connections'} onClick={() => setView('connections')}>⑂ Connections</button><button aria-pressed={view === 'table'} onClick={() => setView('table')}>☷ Full list</button></div></div>
